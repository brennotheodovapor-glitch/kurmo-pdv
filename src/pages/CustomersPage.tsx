...
export default function CustomersPage(){
  ...
  async function load(){
    setLoading(true)
    const{data}=await supabase.from('customers').select('id,name,phone,address,neighborhood,orders_count,total_spent,created_at').order('name')
  ...
  async function doDelete(){
    ...
    if(linked&&linked.length>0){
      setBusy(false)
      setDelC(null)
      alert('Cliente tem pedidos vinculados. Nao pode ser excluido.')
      return
    }
  ...
  async function doAdd(){
    ...
    setBusy(true)
    const{data,error}=await supabase.from('customers').insert({name:af.name.trim(),phone:af.phone.replace(/\D/g,''),address:af.address,neighborhood:af.neighborhood,orders_count:0,total_spent:0}).select().single()
    if(error){alert('Erro: '+error.message);setBusy(false);return}
    setList(prev=>[...prev,data as Cust].sort((a,b)=>a.name.localeCompare(b.name)))
    setAddOpen(false)
    setAf({name:'',phone:'',address:'',neighborhood:''})
    setBusy(false)
  }
  ...
  return(
    <div style={{padding:'16px 12px',maxWidth:640,margin:'0 auto'}}>
      <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:14}}>
        ...
        <button onClick={()=>{setAddOpen(true);setAf({name:'',phone:'',address:'',neighborhood:''})}} style={{padding:'8px 14px',borderRadius:10,background:'#00ff41',border:'none',cursor:'pointer',fontFamily:'Bangers,cursive',fontSize:14,color:'#000',display:'flex',alignItems:'center',gap:6}}>
          <Plus size={14}/>NOVO
        </button>
      </div>
              ...
              <div style={{padding:'0 14px 14px',borderTop:'1px solid #1e1e1e'}}>
                ...
                <div style={{display:'flex',gap:8,marginTop:12}}>
                  ...
                  <button onClick={()=>setDelC(c)}
                    style={{flex:1,padding:'9px',borderRadius:8,border:'1px solid rgba(255,51,51,0.3)',background:'rgba(255,51,51,0.08)',color:'#ff5555',cursor:'pointer',fontSize:12,display:'flex',alignItems:'center',justifyContent:'center',gap:4}}>
                    <Trash2 size={12}/>
                  </button>
                </div>
              </div>
        ...
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.9)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:60,padding:16}}>
          <div style={{background:'#161616',borderRadius:16,padding:24,width:'100%',maxWidth:420,border:'1px solid #2a2a2a',maxHeight:'90vh',overflowY:'auto'}}>
            ...
            <div style={{display:'flex',gap:8,marginTop:18}}>
              ...
              <button onClick={doEdit} disabled={busy} style={{flex:2,padding:'12px',borderRadius:10,border:'none',background:busy?'#333':'#3b82f6',color:busy?'#666':'#fff',cursor:'pointer',fontFamily:'Bangers,cursive',fontSize:16,letterSpacing:1}}>
      ...
      {delC&&(
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.9)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:60,padding:16}}>
          <div style={{background:'#161616',borderRadius:16,padding:24,width:'100%',maxWidth:340,border:'1px solid rgba(255,51,51,0.2)',textAlign:'center'}}>
            ...
            <p style={{fontSize:12,color:'#666',margin:'0 0 18px'}}>Com pedidos: nao pode ser excluido.</p>
            <div style={{display:'flex',gap:8}}>
              ...
              <button onClick={doDelete} disabled={busy} style={{flex:1,padding:'12px',borderRadius:10,border:'none',background:busy?'#333':'#ff5555',color:busy?'#666':'#fff',cursor:'pointer',fontFamily:'Bangers,cursive',fontSize:16,letterSpacing:1}}>
                {busy?'...':'EXCLUIR'}
              </button>
            </div>
          </div>
        ...
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.9)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:60,padding:16}}>
          <div style={{background:'#161616',borderRadius:16,padding:24,width:'100%',maxWidth:420,border:'1px solid #2a2a2a',maxHeight:'90vh',overflowY:'auto'}}>
            ...
            <div style={{display:'flex',gap:8,marginTop:18}}>
              ...
              <button onClick={doAdd} disabled={busy} style={{flex:2,padding:'12px',borderRadius:10,border:'none',background:busy?'#333':'#00ff41',color:busy?'#666':'#000',cursor:'pointer',fontFamily:'Bangers,cursive',fontSize:16,letterSpacing:1}}>
