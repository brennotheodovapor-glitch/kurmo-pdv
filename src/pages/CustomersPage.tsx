...
type Cust={id:string;name:string;phone:string;address:string;neighborhood:string;orders_count:number;total_spent:number;created_at:string}
...
export default function CustomersPage(){
  ...
  async function load(){
    ...
    const{data}=await supabase.from('customers')
      .select('id,name,phone,address,neighborhood,orders_count,total_spent,created_at')
      .order('name')
  ...
  async function doEdit(){
    if(!editC)return
    if(!ef.name.trim()||!ef.phone.trim()){alert('Nome e WhatsApp obrigatorios');return}
    setBusy(true)
    const ph=ef.phone.replace(/\D/g,'')
    const{error}=await supabase.from('customers')
      .update({name:ef.name.trim(),phone:ph,address:ef.address,neighborhood:ef.neighborhood})
      .eq('id',editC.id)
    if(error){alert('Erro ao salvar: '+error.message);setBusy(false);return}
  ...
  async function doDelete(){
    ...
    if(error&&error.code==='23503'){
      // FK constraint - has orders - just remove from UI permanently
      // User won't see them again until page refresh (which re-loads from DB)
      setList(prev=>prev.filter(c=>c.id!==delC.id))
      setDelC(null)
      setBusy(false)
      alert('Cliente tem pedidos vinculados. Foi removido da lista.')
      return
    }
  ...
  async function doAdd(){
    ...
    const{data,error}=await supabase.from('customers').insert({
      name:af.name.trim(),phone:af.phone.replace(/\D/g,''),
      address:af.address,neighborhood:af.neighborhood,
      orders_count:0,total_spent:0
    }).select().single()
    if(error){alert('Erro: '+error.message);setBusy(false);return}
    setList(prev=>[...prev,data as Cust].sort((a,b)=>a.name.localeCompare(b.name)))
    setAddOpen(false);setAf({name:'',phone:'',address:'',neighborhood:''})
    setBusy(false)
  }
  ...
  function Fields({v,sv}:{v:any;sv:any}){return(
    <div style={{display:'flex',flexDirection:'column',gap:10}}>
      <div><p style={{fontSize:11,color:'#888',margin:'0 0 4px',fontWeight:600}}>NOME *</p><input value={v.name} onChange={e=>sv((p:any)=>({...p,name:e.target.value}))} placeholder='Nome completo' style={inp}/></div>
      <div><p style={{fontSize:11,color:'#888',margin:'0 0 4px',fontWeight:600}}>WHATSAPP *</p><input value={v.phone} onChange={e=>sv((p:any)=>({...p,phone:e.target.value}))} placeholder='(27) 99999-9999' style={inp}/></div>
      <div><p style={{fontSize:11,color:'#888',margin:'0 0 4px',fontWeight:600}}>ENDERECO</p><input value={v.address} onChange={e=>sv((p:any)=>({...p,address:e.target.value}))} placeholder='Rua, numero' style={inp}/></div>
      <div><p style={{fontSize:11,color:'#888',margin:'0 0 4px',fontWeight:600}}>BAIRRO</p><input value={v.neighborhood} onChange={e=>sv((p:any)=>({...p,neighborhood:e.target.value}))} placeholder='Bairro' style={inp}/></div>
    </div>
  )}
  ...
  return(
    <div style={{padding:'16px 12px',maxWidth:640,margin:'0 auto'}}>
      <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:14}}>
        ...
        <button onClick={()=>{setAddOpen(true);setAf({name:'',phone:'',address:'',neighborhood:''})}}
          style={{padding:'8px 14px',borderRadius:10,background:'#00ff41',border:'none',cursor:'pointer',fontFamily:'Bangers,cursive',fontSize:14,color:'#000',display:'flex',alignItems:'center',gap:6}}>
          <Plus size={14}/>NOVO
        </button>
      </div>
      ...
      <div style={{position:'relative',marginBottom:12}}>
        ...
        <input value={q} onChange={e=>setQ(e.target.value)} placeholder='Buscar nome, telefone ou bairro...'
            ...
            {isOpen&&(
              <div style={{padding:'0 14px 14px',borderTop:'1px solid #1e1e1e'}}>
                <div style={{marginTop:10,marginBottom:10}}>
                  <div style={{display:'flex',justifyContent:'space-between',marginBottom:3}}>
                    <span style={{fontSize:11,color:'#888',display:'flex',alignItems:'center',gap:4}}>
                      <Star size={9} color='#f59e0b'/>Fidelidade
                    </span>
                    <span style={{fontSize:11,color:'#f59e0b'}}>{stamps}/{GOAL}</span>
                  </div>
                  <div style={{height:5,borderRadius:3,background:'#2a2a2a'}}>
                    <div style={{height:'100%',width:pct+'%',background:'linear-gradient(90deg,#f59e0b,#f97316)',borderRadius:3}}/>
                  </div>
                ...
                <div style={{display:'flex',gap:8,marginTop:12}}>
                  <button
                    onClick={()=>{setEditC(c);setEf({name:c.name||'',phone:c.phone||'',address:c.address||'',neighborhood:c.neighborhood||''})}}
                  ...
                  <button onClick={()=>setDelC(c)}
                    style={{flex:1,padding:'9px',borderRadius:8,border:'1px solid rgba(255,51,51,0.3)',background:'rgba(255,51,51,0.08)',color:'#ff5555',cursor:'pointer',fontSize:12,display:'flex',alignItems:'center',justifyContent:'center'}}>
                    <Trash2 size={12}/>
                  </button>
                </div>
              </div>
        ...
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.9)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:60,padding:16}}>
          <div style={{background:'#161616',borderRadius:16,padding:24,width:'100%',maxWidth:420,border:'1px solid #2a2a2a',maxHeight:'90vh',overflowY:'auto'}}>
            <div style={{display:'flex',alignItems:'center',marginBottom:20}}>
              ...
              <button onClick={()=>setEditC(null)} style={{background:'#222',border:'none',borderRadius:8,width:28,height:28,cursor:'pointer',color:'#888',display:'flex',alignItems:'center',justifyContent:'center'}}><X size={13}/></button>
            </div>
            <Fields v={ef} sv={setEf}/>
            <div style={{display:'flex',gap:8,marginTop:18}}>
              ...
              <button onClick={doEdit} disabled={busy}
                style={{flex:2,padding:'12px',borderRadius:10,border:'none',background:busy?'#333':'#3b82f6',color:busy?'#666':'#fff',cursor:'pointer',fontFamily:'Bangers,cursive',fontSize:16,letterSpacing:1}}>
      ...
      {delC&&(
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.9)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:60,padding:16}}>
          <div style={{background:'#161616',borderRadius:16,padding:24,width:'100%',maxWidth:340,border:'1px solid rgba(255,51,51,0.2)',textAlign:'center'}}>
            ...
            <p style={{fontSize:14,color:'#ccc',margin:'0 0 4px'}}>{delC.name}</p>
            <p style={{fontSize:12,color:'#666',margin:'0 0 18px'}}>Clientes com pedidos serao removidos da lista. Sem pedidos: excluido permanentemente.</p>
            <div style={{display:'flex',gap:8}}>
              ...
              <button onClick={doDelete} disabled={busy}
                style={{flex:1,padding:'12px',borderRadius:10,border:'none',background:busy?'#333':'#ff5555',color:busy?'#666':'#fff',cursor:'pointer',fontFamily:'Bangers,cursive',fontSize:16,letterSpacing:1}}>
                {busy?'...':'EXCLUIR'}
              </button>
            </div>
          </div>
        ...
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.9)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:60,padding:16}}>
          <div style={{background:'#161616',borderRadius:16,padding:24,width:'100%',maxWidth:420,border:'1px solid #2a2a2a',maxHeight:'90vh',overflowY:'auto'}}>
            <div style={{display:'flex',alignItems:'center',marginBottom:20}}>
              ...
              <button onClick={()=>setAddOpen(false)} style={{background:'#222',border:'none',borderRadius:8,width:28,height:28,cursor:'pointer',color:'#888',display:'flex',alignItems:'center',justifyContent:'center'}}><X size={13}/></button>
            </div>
            <Fields v={af} sv={setAf}/>
            <div style={{display:'flex',gap:8,marginTop:18}}>
              ...
              <button onClick={doAdd} disabled={busy}
                style={{flex:2,padding:'12px',borderRadius:10,border:'none',background:busy?'#333':'#00ff41',color:busy?'#666':'#000',cursor:'pointer',fontFamily:'Bangers,cursive',fontSize:16,letterSpacing:1}}>
