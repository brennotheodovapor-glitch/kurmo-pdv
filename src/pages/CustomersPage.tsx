...
import{Search,Edit2,Trash2,X,MapPin,User,Star,ChevronDown,ChevronUp,Plus,AlertTriangle}from 'lucide-react'

const fmt=(v:number)=>new Intl.NumberFormat('pt-BR',{style:'currency',currency:'BRL'}).format(v||0)

type Customer={
  id:string;name:string;phone:string;email:string;address:string;neighborhood:string;
...
export default function CustomersPage(){
  ...
  async function load(){
    ...
    const{data}=await supabase
      .from('customers')
      .select('id,name,phone,email,address,neighborhood,orders_count,total_spent,created_at')
      .order('name')

    const custs=data||[]
    setCustomers(custs as Customer[])

    // Load real completed order count per customer
    if(custs.length>0){
  ...
  async function saveEdit(){
    if(!editModal)return
    if(!editForm.name.trim()||!editForm.phone.trim()){alert('Nome e telefone obrigatorios');return}
    setSaving(true)
  ...
  async function deleteCustomer(){
    ...
    setSaving(true)
    const{data:linked}=await supabase.from('orders').select('id').eq('customer_id',deleteModal.id).limit(1)
    if(linked&&linked.length>0){
      setSaving(false)
      setDeleteModal(null)
      alert('Este cliente tem pedidos e nao pode ser excluido. Para ocultar, edite o nome.')
      return
    }
  ...
  async function addCustomer(){
    if(!addForm.name.trim()||!addForm.phone.trim()){alert('Nome e telefone obrigatorios');return}
    setSaving(true)
  ...
  function EditForm({form,setForm}:{form:typeof BLANK;setForm:any}){
    return(
      <div style={{display:'flex',flexDirection:'column',gap:12}}>
        <div><span style={lbl}>Nome completo *</span><input value={form.name} onChange={e=>setForm((p:any)=>({...p,name:e.target.value}))} placeholder='Nome' style={inp}/></div>
        ...
        <div><span style={lbl}>Endereco</span><input value={form.address} onChange={e=>setForm((p:any)=>({...p,address:e.target.value}))} placeholder='Rua, numero' style={inp}/></div>
  ...
  return(
    <div style={{padding:'16px 12px',maxWidth:680,margin:'0 auto'}}>
      <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:14}}>
        ...
        <button onClick={()=>{setAddModal(true);setAddForm(BLANK)}} style={{display:'flex',alignItems:'center',gap:6,padding:'8px 14px',borderRadius:10,background:'#00ff41',border:'none',cursor:'pointer',fontFamily:'Bangers,cursive',fontSize:14,color:'#000'}}>
          <Plus size={14}/>NOVO
        </button>
      </div>
      ...
      filtered.map(cust=>{
        const stamps=loyaltyMap[cust.id]||0
        const pct=Math.min(100,Math.round((stamps/LOYALTY_GOAL)*100))
        const isOpen=expanded===cust.id
        return(
          <div key={cust.id} style={{background:'#161616',borderRadius:12,marginBottom:8,border:'1px solid #1e1e1e'}}>
            <div style={{display:'flex',alignItems:'center',gap:10,padding:'12px 14px',cursor:'pointer'}} onClick={()=>setExpanded(isOpen?null:cust.id)}>
              ...
              <div style={{textAlign:'right',flexShrink:0}}>
                ...
                <p style={{fontSize:10,color:'#555',margin:0}}>{stamps} pedidos finalizados</p>
              </div>
              ...
              <div style={{padding:'0 14px 14px',borderTop:'1px solid #1e1e1e'}}>
                <div style={{marginTop:10,marginBottom:12}}>
                  <div style={{display:'flex',justifyContent:'space-between',marginBottom:4}}>
                    <span style={{fontSize:11,color:'#888',display:'flex',alignItems:'center',gap:4}}><Star size={10} color='#f59e0b'/>Fidelidade ({stamps}/{LOYALTY_GOAL} compras)</span>
                  ...
                  {stamps>=LOYALTY_GOAL&&<p style={{fontSize:11,color:'#f59e0b',marginTop:4}}>Cliente atingiu a meta!</p>}
                </div>
                ...
                <div style={{display:'flex',gap:8,marginTop:12}}>
                  <button onClick={()=>{setEditModal(cust);setEditForm({name:cust.name||'',phone:cust.phone||'',address:cust.address||'',neighborhood:cust.neighborhood||'',notes:''})}}
        ...
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.88)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:50,padding:16}}>
          <div style={{background:'#161616',borderRadius:16,padding:24,width:'100%',maxWidth:360,border:'1px solid rgba(255,51,51,0.2)'}}>
            <div style={{textAlign:'center',marginBottom:20}}>
              ...
              <p style={{fontSize:12,color:'#666',marginTop:6}}>Clientes sem pedidos serao excluidos permanentemente.</p>
            </div>
            <div style={{display:'flex',gap:8}}>
              ...
              <button onClick={deleteCustomer} disabled={saving} style={{flex:1,padding:'12px',borderRadius:10,border:'none',background:saving?'#2a2a2a':'#ff5555',color:saving?'#555':'#fff',cursor:saving?'not-allowed':'pointer',fontFamily:'Bangers,cursive',fontSize:16}}>
                {saving?'...':'EXCLUIR'}
              </button>
            </div>
          </div>
        ...
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.88)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:50,padding:16}}>
          <div style={{background:'#161616',borderRadius:16,padding:24,width:'100%',maxWidth:420,border:'1px solid #2a2a2a',maxHeight:'90vh',overflowY:'auto'}}>
            ...
            <div style={{display:'flex',gap:8,marginTop:20}}>
              ...
              <button onClick={addCustomer} disabled={saving} style={{flex:2,padding:'12px',borderRadius:10,border:'none',background:saving?'#2a2a2a':'#00ff41',color:saving?'#555':'#000',cursor:saving?'not-allowed':'pointer',fontFamily:'Bangers,cursive',fontSize:16}}>
