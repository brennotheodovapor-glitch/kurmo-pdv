...
import{Search,Edit2,Trash2,X,Check,MapPin,User,Star,ChevronDown,ChevronUp,Plus,AlertTriangle}from 'lucide-react'
...
export default function CustomersPage(){
  ...
  async function load(){
    ...
    const{data}=await supabase
      .from('customers')
      .select('id,name,phone,address,neighborhood,notes,active,loyalty_stamps,loyalty_goal,created_at')
      .order('name')
    // Enrich with real completed order counts
    if(data&&data.length>0){
      ...
      const enriched=data.map((c:any)=>({
        ...c,
        total_orders:orderMap[c.id]?.count||0,
  ...
  async function deleteCustomer(){
    ...
    setSaving(true)
    // Check if customer has orders — FK constraint prevents hard delete
    const{data:linked}=await supabase.from('orders').select('id').eq('customer_id',deleteModal.id).limit(1)
    if(linked&&linked.length>0){
      // Soft delete — deactivate instead
      await supabase.from('customers').update({active:false}).eq('id',deleteModal.id)
      setCustomers(prev=>prev.map(c=>c.id===deleteModal.id?{...c,active:false}:c))
      setDeleteModal(null)
      setSaving(false)
      alert('Este cliente tem pedidos vinculados e foi desativado em vez de excluido.')
      return
    }
  ...
  async function addCustomer(){
    ...
    if(data)setCustomers(prev=>[...prev,{...data,total_orders:0,total_spent:0,loyalty_stamps:0}].sort((a,b)=>a.name.localeCompare(b.name)))
    setAddModal(false)
    setAddForm(BLANK)
    setSaving(false)
  }

  async function toggleActive(cust:Customer){
    await supabase.from('customers').update({active:!cust.active}).eq('id',cust.id)
    setCustomers(prev=>prev.map(c=>c.id===cust.id?{...c,active:!cust.active}:c))
  }
  ...
  function EditForm({form,setForm}:{form:typeof BLANK;setForm:any}){
    return(
      <div style={{display:'flex',flexDirection:'column',gap:12}}>
        ...
        <div><span style={lbl}>Observacoes</span><input value={form.notes} onChange={e=>setForm((p:any)=>({...p,notes:e.target.value}))} placeholder='Ex: alergico a...' style={inp}/></div>
      </div>
    )
  }
              ...
              <div style={{padding:'0 14px 14px',borderTop:'1px solid #1e1e1e'}}>
                <div style={{marginTop:10,marginBottom:12}}>
                  <div style={{display:'flex',justifyContent:'space-between',marginBottom:4}}>
                    <span style={{fontSize:11,color:'#888',display:'flex',alignItems:'center',gap:4}}><Star size={10} color='#f59e0b'/>Fidelidade ({stamps}/{goal} compras)</span>
                  ...
                  {stamps>=goal&&<p style={{fontSize:11,color:'#f59e0b',marginTop:4}}>Parabens! Cliente atingiu a meta de fidelidade!</p>}
                </div>
                ...
                {!cust.active&&<p style={{fontSize:11,color:'#ff5555',margin:'6px 0 0',display:'flex',alignItems:'center',gap:4}}><AlertTriangle size={10}/>Cliente desativado</p>}
        ...
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.88)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:50,padding:16}}>
          <div style={{background:'#161616',borderRadius:16,padding:24,width:'100%',maxWidth:360,border:'1px solid rgba(255,51,51,0.2)'}}>
            <div style={{textAlign:'center',marginBottom:20}}>
              ...
              <p style={{fontSize:12,color:'#666',marginTop:6}}>Se tiver pedidos vinculados, sera desativado. Caso contrario, excluido permanentemente.</p>
            </div>
