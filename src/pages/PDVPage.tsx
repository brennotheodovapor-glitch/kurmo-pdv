...
import{Plus,Minus,X,Check,Search,ShoppingCart,AlertTriangle,User,Printer,Tag}from 'lucide-react'
...
export default function PDVPage({sellerId:propSellerId,sellerName:propSellerName}:Props={}){
  ...
  async function applyCoupon(){
    ...
    if(!data){toast.error('Cupom invalido ou expirado');setCouponData(null);setCouponLoading(false);return}
    ...
    if(data.min_order&&subtotal<data.min_order){toast.error('Pedido minimo: '+new Intl.NumberFormat('pt-BR',{style:'currency',currency:'BRL'}).format(data.min_order));setCouponData(null);setCouponLoading(false);return}
    setCouponData(data)
  ...
  function printReceipt(order:any,orderItems:any[]){
    ...
    const lines=orderItems.map(i=>'<tr><td>'+i.qty+'x '+i.name+'</td><td style="text-align:right">'+fmt2(i.price*i.qty)+'</td></tr>').join('')
    w.document.write('<html><head><title>Cupom #'+order.order_number+'</title><style>body{font-family:monospace;font-size:13px;max-width:320px;margin:0 auto;padding:16px}h2{text-align:center;font-size:18px}table{width:100%;border-collapse:collapse}td{padding:3px 0}hr{border:1px dashed #ccc}.total{font-size:16px;font-weight:bold}.center{text-align:center}</style></head><body>')
    ...
    if(order.discount>0)w.document.write('<p>Desconto: -'+fmt2(order.discount)+'</p>')
    if(order.coupon_code)w.document.write('<p>Cupom ('+order.coupon_code+'): -'+fmt2(order.coupon_discount||0)+'</p>')
    w.document.write('<p class="total">TOTAL: '+fmt2(order.total)+'</p><hr/>')
    w.document.write('<p class="center">Obrigado pela preferencia!<br>UZT 027</p></body></html>')
    w.document.close()
  ...
  return(
    <div style={{height:'100%',display:'flex',background:'var(--bg)',overflow:'hidden',position:'relative'}}>

      {/* Overlay caixa fechado */}
      {!cash.isLoading&&!cash.isOpen&&(
        <div style={{position:'absolute',inset:0,background:'rgba(8,12,8,0.97)',backdropFilter:'blur(8px)',display:'flex',alignItems:'center',justifyContent:'center',flexDirection:'column',gap:24,zIndex:99,padding:20}}>
          ...
          <div style={{textAlign:'center'}}>
            ...
            <p style={{fontSize:15,color:'var(--muted)',maxWidth:300,lineHeight:1.5}}>Abra o caixa para comecar a registrar vendas do dia.</p>
          </div>
          ...
          {filtered.map(p=>(
            <button key={p.id} onClick={()=>addToCart(p)} disabled={p.stock===0} style={{background:'var(--card)',border:'1px solid var(--border)',borderRadius:12,padding:10,cursor:p.stock===0?'not-allowed':'pointer',textAlign:'left',transition:'all 0.15s',opacity:p.stock===0?0.4:1}} className='card-hover'>
              {p.image_url&&p.image_url.startsWith('http')
                ?<img src={p.image_url} alt={p.name} style={{width:'100%',height:72,objectFit:'cover',borderRadius:8,marginBottom:6}} onError={e=>{(e.target as HTMLImageElement).style.display='none'}}/>
                :<div style={{width:'100%',height:72,background:'var(--surface)',borderRadius:8,marginBottom:6}}/>
              }
              ...
              <p style={{fontSize:10,color:p.stock===0?'#ff3333':p.stock<=5?'#ffaa00':'var(--muted)'}}>{p.stock===0?'SEM ESTOQUE':'Estoque: '+p.stock}</p>
            </button>
          ))}
          ...
          <div style={{marginBottom:8}}>
            ...
            {payments.map((pm,i)=>(
              <div key={i} style={{display:'flex',gap:5,marginBottom:5,alignItems:'center'}}>
                ...
                <button onClick={()=>fillRemaining(i)} style={{fontSize:9,color:'var(--neon)',background:'var(--neon-glow)',border:'1px solid var(--neon-dim)',borderRadius:5,padding:'4px 5px',cursor:'pointer',whiteSpace:'nowrap'}}>Resto</button>
