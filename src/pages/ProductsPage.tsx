...
const EMPTY:Omit<Product,'id'>={name:'',price:0,cost_price:0,stock:0,stock_alert:5,category_id:null,active:true,image_url:'',description:''}
...
export default function ProductsPage(){
  ...
  async function loadData(){
    ...
    const[p,c]=await Promise.all([
      supabase.from('products').select('id,name,price,cost_price,stock,stock_alert,category_id,active,image_url,description').order('name'),
  ...
  const openE=(p:Product)=>{
    setEdit(p)
    setForm({name:p.name,price:p.price,cost_price:p.cost_price,stock:p.stock,stock_alert:p.stock_alert??5,category_id:p.category_id,active:p.active,image_url:p.image_url||'',description:p.description||''})
            ...
            <div style={{display:'flex',gap:14,flexWrap:'wrap',marginBottom:14}}>
              ...
              <div style={{flex:1,minWidth:180,display:'flex',flexDirection:'column',gap:10}}>
                ...
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr 1fr',gap:7}}>
                  ...
                  <div><label style={{fontSize:10,color:'var(--muted)',display:'block',marginBottom:2}}>ALERTA</label><input type='number' min='1' value={(form.stock_alert??5)===5&&!(form as any)._alertTouched?'':form.stock_alert??5} onChange={e=>setForm(f=>({...f,stock_alert:parseInt(e.target.value)||5}))} placeholder='5'/></div>
                </div>
              </div>
            </div>
