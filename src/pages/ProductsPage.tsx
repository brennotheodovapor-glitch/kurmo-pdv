...
export default function ProductsPage(){
  ...
  async function save(){
    ...
    if(uploading){toast.error('Aguarde o upload da foto terminar');return}
    if(edit){
    ...
    <div style={{height:'100%',display:'flex',flexDirection:'column',background:'var(--bg)'}}>
      <div style={{padding:'12px 16px',borderBottom:'1px solid var(--border)',background:'var(--surface)',display:'flex',alignItems:'center',gap:10,flexWrap:'wrap'}}>
        ...
        <div style={{display:'flex',gap:5,flexWrap:'wrap'}}>
          ...
          {categories.map(c=>(
            <button key={c.id} onClick={()=>setCatFilter(c.id===catFilter?null:c.id)} style={{padding:'4px 10px',borderRadius:8,border:catFilter===c.id?'1px solid var(--neon)':'1px solid var(--border)',background:catFilter===c.id?'var(--neon-glow)':'transparent',color:catFilter===c.id?'var(--neon)':'var(--muted)',cursor:'pointer',fontSize:12}}>
              {c.name}
            </button>
          ))}
        </div>
          ...
          <div className="card" style={{overflow:'hidden'}}>
            <table style={{width:'100%',borderCollapse:'collapse'}}>
              <thead><tr style={{borderBottom:'1px solid var(--border)'}}>
                ...
                <th style={{padding:'9px 12px',textAlign:'left',fontSize:11,color:'var(--muted)',fontWeight:600,letterSpacing:1}} className="hide-mobile">CAT</th>
                <th style={{padding:'9px 12px',textAlign:'left',fontSize:11,color:'var(--muted)',fontWeight:600,letterSpacing:1}}>PRECO</th>
                <th style={{padding:'9px 12px',textAlign:'left',fontSize:11,color:'var(--muted)',fontWeight:600,letterSpacing:1}} className="hide-mobile">MARGEM</th>
                <th style={{padding:'9px 12px',textAlign:'left',fontSize:11,color:'var(--muted)',fontWeight:600,letterSpacing:1}}>ESTQ</th>
                ...
                <tr key={p.id} style={{borderBottom:'1px solid rgba(26,46,26,0.5)'}}>
                  ...
                  <td style={{padding:'9px 12px'}} className="hide-mobile">
                    <span style={{fontSize:11,fontWeight:700,padding:'2px 7px',borderRadius:20,
                      background:parseInt(mar(p))>=40?'rgba(0,255,65,0.1)':parseInt(mar(p))>=25?'rgba(255,170,0,0.1)':'rgba(255,51,51,0.1)',
                      color:parseInt(mar(p))>=40?'var(--neon)':parseInt(mar(p))>=25?'#ffaa00':'#ff3333'}}>
                      {mar(p)}%
                    </span>
                  </td>
            ...
            <div style={{display:'flex',gap:16,flexWrap:'wrap',marginBottom:14}}>
              ...
              <div style={{flexShrink:0}}>
                <div
                  ...
                  style={{width:100,height:100,borderRadius:12,border:'2px dashed var(--border)',overflow:'hidden',cursor:uploading?'wait':'pointer',background:'var(--surface)',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:6,position:'relative',transition:'border-color 0.2s'}}
                >
                  {preview&&preview.length>0?(
                    ...
                    <>
                      ...
                      <p style={{fontSize:10,color:'var(--muted)',textAlign:'center',lineHeight:1.3}}>{uploading?'Enviando...':'Clique para
adicionar foto'}</p>
                    </>
                  )}
                  {uploading&&(
                    <div style={{position:'absolute',inset:0,background:'rgba(0,0,0,0.5)',display:'flex',alignItems:'center',justifyContent:'center'}}>
                      <p style={{color:'var(--neon)',fontSize:11,fontWeight:600}}>Enviando...</p>
                    </div>
                  )}
                </div>
                <input ref={fileRef} type="file" accept="image/jpeg,image/png,image/webp,image/gif" onChange={handleImg} style={{display:'none'}}/>
                {preview&&(
                  <button onClick={()=>{setPreview(null);setForm(f=>({...f,image_url:''}))}} style={{fontSize:10,color:'#ff3333',background:'none',border:'none',cursor:'pointer',marginTop:4,display:'block',width:'100%',textAlign:'center'}}>
                    Remover
                  </button>
                )}
              </div>
              ...
              <div style={{flex:1,minWidth:180,display:'flex',flexDirection:'column',gap:11}}>
                <div>
                  <label style={{fontSize:11,color:'var(--muted)',display:'block',marginBottom:4,letterSpacing:1}}>NOME DO PRODUTO</label>
                  <input value={form.name} onChange={e=>setForm(f=>({...f,name:e.target.value}))} placeholder="Ex: Elfbar BC5000"/>
                </div>
                <div>
                  <label style={{fontSize:11,color:'var(--muted)',display:'block',marginBottom:4,letterSpacing:1}}>DESCRICAO (opcional)</label>
                  <input value={form.description||''} onChange={e=>setForm(f=>({...f,description:e.target.value}))} placeholder="Descricao breve"/>
                </div>
                <div>
                  <label style={{fontSize:11,color:'var(--muted)',display:'block',marginBottom:4,letterSpacing:1}}>CATEGORIA</label>
                ...
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:8}}>
                  <div>
                    <label style={{fontSize:10,color:'var(--muted)',display:'block',marginBottom:3,letterSpacing:1}}>PRECO VENDA</label>
                    <input type="number" min="0" step="0.01" value={form.price===0?'':form.price} onChange={e=>setForm(f=>({...f,price:e.target.value===''?0:parseFloat(e.target.value)||0}))} placeholder="0,00"/>
                  </div>
                  <div>
                    <label style={{fontSize:10,color:'var(--muted)',display:'block',marginBottom:3,letterSpacing:1}}>PRECO CUSTO</label>
                    <input type="number" min="0" step="0.01" value={form.cost_price===0?'':form.cost_price} onChange={e=>setForm(f=>({...f,cost_price:e.target.value===''?0:parseFloat(e.target.value)||0}))} placeholder="0,00"/>
                  </div>
                  <div>
                    <label style={{fontSize:10,color:'var(--muted)',display:'block',marginBottom:3,letterSpacing:1}}>ESTOQUE</label>
                    <input type="number" min="0" value={form.stock===0?'':form.stock} onChange={e=>setForm(f=>({...f,stock:e.target.value===''?0:parseInt(e.target.value)||0}))} placeholder="0"/>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div style={{display:'flex',gap:10}}>
              ...
              <button onClick={save} className="btn-neon-fill" style={{flex:2,fontSize:14}} disabled={uploading}>
                <Check size={13} style={{display:'inline',marginRight:5}}/>{uploading?'AGUARDE...':'SALVAR PRODUTO'}
              </button>
            </div>
          </div>
        </div>
