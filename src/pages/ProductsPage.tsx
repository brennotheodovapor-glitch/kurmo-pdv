...
const EMPTY={name:'',price:0,cost_price:0,stock:0,category_id:null as string|null,active:true,image_url:'',description:''}
...
export default function ProductsPage(){
  ...
  const openE=(p:Product)=>{setEdit(p);setForm({...EMPTY,...p});setPreview(p.image_url||null);setModal(true)}
  async function handleImg(e:React.ChangeEvent<HTMLInputElement>){
    ...
    setUploading(true)
    try{
      const fn='prod_'+Date.now()+'.'+file.name.split('.').pop()
  ...
  async function save(){
    ...
    const payload={name:form.name,description:form.description,price:form.price,cost_price:form.cost_price,stock:form.stock,category_id:form.category_id,active:form.active,image_url:form.image_url||null}
    if(edit){
  ...
  async function del(id:string){if(!confirm('Remover produto?'))return;await supabase.from('products').delete().eq('id',id);toast.success('Removido');load()}
  const getCat=(id:string|null)=>categories.find(c=>c.id===id)
  const mar=(p:{price:number;cost_price:number})=>p.price>0?((p.price-p.cost_price)/p.price*100).toFixed(0):'0'
  return(<div style={{height:'100%',display:'flex',flexDirection:'column',background:'var(--bg)'}}>
    <div style={{padding:'14px 20px',borderBottom:'1px solid var(--border)',background:'var(--surface)',display:'flex',alignItems:'center',gap:12,flexWrap:'wrap'}}>
      ...
      <div style={{display:'flex',gap:5,flexWrap:'wrap'}}>
        <button onClick={()=>setCatF(null)} style={{padding:'5px 10px',borderRadius:8,border:!catF?'1px solid var(--neon)':'1px solid var(--border)',background:!catF?'var(--neon-glow)':'transparent',color:!catF?'var(--neon)':'var(--muted)',cursor:'pointer',fontSize:12}}>Todos</button>
        {categories.map(c=><button key={c.id} onClick={()=>setCatF(c.id===catF?null:c.id)} style={{padding:'5px 10px',borderRadius:8,border:catF===c.id?'1px solid var(--neon)':'1px solid var(--border)',background:catF===c.id?'var(--neon-glow)':'transparent',color:catF===c.id?'var(--neon)':'var(--muted)',cursor:'pointer',fontSize:12}}>{c.icon} {c.name}</button>)}
      </div>
      ...
      <div className="card" style={{overflow:'hidden'}}>
        <table style={{width:'100%',borderCollapse:'collapse'}}>
          <thead><tr style={{borderBottom:'1px solid var(--border)'}}>
            {['PRODUTO','CATEGORIA','PRECO','CUSTO','MARGEM','ESTOQUE','STATUS',''].map(h=><th key={h} style={{padding:'10px 14px',textAlign:'left',fontSize:11,color:'var(--muted)',fontWeight:600,letterSpacing:1}}>{h}</th>)}
          </tr></thead>
          <tbody>{filtered.map(p=>(
            <tr key={p.id} style={{borderBottom:'1px solid rgba(26,46,26,0.5)'}}>
              ...
              <td style={{padding:'10px 14px',fontSize:12,color:p.stock===0?'#ff3333':p.stock<=5?'#ffaa00':'var(--muted)'}}>{p.stock<=5&&p.stock>0&&<AlertTriangle size={11} style={{display:'inline',marginRight:3}}/>}{p.stock} un</td>
              <td style={{padding:'10px 14px'}}><span style={{fontSize:11,padding:'3px 8px',borderRadius:20,background:p.active?'rgba(0,255,65,0.1)':'rgba(77,77,77,0.1)',color:p.active?'var(--neon)':'var(--muted)'}}>{p.active?'ATIVO':'INATIVO'}</span></td>
              <td style={{padding:'10px 14px'}}>
          ...
          ))}{filtered.length===0&&<tr><td colSpan={8} style={{textAlign:'center',padding:48,color:'var(--muted)'}}><Package size={32} style={{marginBottom:8,opacity:0.3,display:'block',margin:'0 auto 8px'}}/><p>Nenhum produto</p></td></tr>}</tbody>
        </table>
      </div>)}
    </div>
    {modal&&(
      <div className="animate-fade-in" style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.85)',backdropFilter:'blur(4px)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:50}}>
        <div className="animate-slide-in card" style={{width:'100%',maxWidth:540,padding:28,margin:16,border:'1px solid var(--border-bright)',boxShadow:'0 0 40px rgba(0,255,65,0.15)',maxHeight:'90vh',overflowY:'auto'}}>
          ...
          <div style={{display:'flex',gap:16,marginBottom:14}}>
            <div style={{flexShrink:0}}>
              <div onClick={()=>fileRef.current?.click()} style={{width:100,height:100,borderRadius:12,border:'2px dashed var(--border)',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',cursor:'pointer',overflow:'hidden',background:'var(--surface)',transition:'border-color 0.2s'}} className="card-hover">
              ...
              {preview&&<button onClick={()=>{setPreview(null);setForm(f=>({...f,image_url:''}))}} style={{fontSize:10,color:'#ff3333',background:'none',border:'none',cursor:'pointer',width:'100%',marginTop:4,textAlign:'center'}}>Remover foto</button>}
            </div>
            <div style={{flex:1,display:'flex',flexDirection:'column',gap:10}}>
              <div><label style={{fontSize:11,color:'var(--muted)',display:'block',marginBottom:4,letterSpacing:1}}>NOME DO PRODUTO</label><input value={form.name} onChange={e=>setForm(f=>({...f,name:e.target.value}))} placeholder="Ex: Elfbar BC5000"/></div>
              <div><label style={{fontSize:11,color:'var(--muted)',display:'block',marginBottom:4,letterSpacing:1}}>DESCRICAO</label><input value={form.description||''} onChange={e=>setForm(f=>({...f,description:e.target.value}))} placeholder="Descricao curta (opcional)"/></div>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:10,marginBottom:10}}>
            <div><label style={{fontSize:11,color:'var(--muted)',display:'block',marginBottom:4,letterSpacing:1}}>PRECO VENDA (R$)</label><input type="number" min="0" step="0.01" value={form.price||''} onChange={e=>setForm(f=>({...f,price:parseFloat(e.target.value)||0}))}/></div>
            <div><label style={{fontSize:11,color:'var(--muted)',display:'block',marginBottom:4,letterSpacing:1}}>PRECO CUSTO (R$)</label><input type="number" min="0" step="0.01" value={form.cost_price||''} onChange={e=>setForm(f=>({...f,cost_price:parseFloat(e.target.value)||0}))}/></div>
            <div><label style={{fontSize:11,color:'var(--muted)',display:'block',marginBottom:4,letterSpacing:1}}>ESTOQUE (un)</label><input type="number" min="0" value={form.stock||''} onChange={e=>setForm(f=>({...f,stock:parseInt(e.target.value)||0}))}/></div>
          </div>
          {form.price>0&&form.cost_price>=0&&(
            <div style={{padding:'10px 14px',background:'var(--surface)',borderRadius:8,display:'flex',justifyContent:'space-between',marginBottom:10}}>
              <span style={{fontSize:12,color:'var(--muted)'}}>Margem de lucro</span>
              <span style={{fontSize:13,fontWeight:700,color:parseFloat(mar(form))>=40?'var(--neon)':parseFloat(mar(form))>=25?'#ffaa00':'#ff3333',fontFamily:'JetBrains Mono,monospace'}}>{mar(form)}% — Lucro: {fmt(form.price-form.cost_price)}</span>
            </div>
          )}
          <div style={{display:'flex',alignItems:'center',gap:10,padding:'10px 14px',background:'var(--surface)',borderRadius:8,marginBottom:16}}>
            <input type="checkbox" id="pactive" checked={form.active} onChange={e=>setForm(f=>({...f,active:e.target.checked}))} style={{width:16,height:16,accentColor:'var(--neon)'}}/>
            <label htmlFor="pactive" style={{fontSize:13,color:'var(--text)',cursor:'pointer'}}>Produto ativo (aparece no PDV)</label>
          </div>
