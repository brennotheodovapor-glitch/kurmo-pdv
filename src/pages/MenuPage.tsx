...
export default function MenuPage(){
  ...
  async function loadOrders(){
    setLoading(true)
    const{data}=await supabase.from('orders').select('*').eq('type','delivery').order('created_at',{ascending:false}).limit(60)
    setOrders(data||[])
    setLoading(false)
  }
          ...
          <div style={{maxWidth:760,margin:'0 auto'}}>
            <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:16,flexWrap:'wrap',gap:10}}>
              <div style={{display:'flex',gap:16}}>
                {[{s:'pending',l:'Aguardando',c:'#ffaa00'},{s:'accepted',l:'Aceito',c:'#06b6d4'},{s:'preparing',l:'Preparo',c:'#7c3aed'},{s:'delivering',l:'Entrega',c:'#f59e0b'}].map(st=>{
              ...
              <div key={o.id} className='card' style={{marginBottom:10,overflow:'hidden',borderLeft:'3px solid '+(SC[o.status]||'var(--border)'),background:o.status==='pending'?'rgba(255,170,0,0.03)':'var(--card)'}}>
                ...
                {expanded===o.id&&(
                  <div style={{padding:'0 16px 14px',borderTop:'1px solid var(--border)'}}>
                    {o.notes&&(
                      <div style={{padding:'8px 12px',background:'var(--surface)',borderRadius:8,marginBottom:10,fontSize:12,color:'var(--text)',display:'flex',alignItems:'flex-start',gap:6}}>
                    ...
                    <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
                      ...
                      {!['cancelled','delivered','completed'].includes(o.status)&&(
                        <button onClick={()=>updateStatus(o.id,'cancelled')} style={{padding:'9px 14px',borderRadius:8,border:'1px solid #ff3333',background:'rgba(255,51,51,0.1)',color:'#ff3333',cursor:'pointer',fontSize:13,fontFamily:'Bangers,cursive'}}>
                          Cancelar
                        </button>
                      )}
                    </div>
